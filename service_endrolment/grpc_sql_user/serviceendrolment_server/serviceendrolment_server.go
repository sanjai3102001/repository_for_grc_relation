...
func (server *UserManagementServer) CreateNewUser(ctx context.Context, in *pb.NewUser) (*pb.User, error) {
	...
	log.Printf("Received: %v %v", in.GetServiceId(), in.GetUserId())

	created_user := &pb.User{ServiceId: in.GetServiceId(), UserId: in.GetUserId()}
	tx, err := server.conn.Begin(context.Background())
	if err != nil {
		log.Fatalf("conn.Begin failed: %v", err)
	}

	_, err = tx.Exec(context.Background(), "insert into service_enrollment(service_id, user_id) values ($1,$2)",
		created_user.ServiceId, created_user.UserId)
	if err != nil {
...
func (server *UserManagementServer) GetUsers(ctx context.Context, in *pb.GetUsersParams) (*pb.UsersList, error) {
	...
	rows, err := server.conn.Query(context.Background(), "select * from service_enrollment")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		user := pb.User{}
		err = rows.Scan(&user.ServiceId, &user.UserId)
		if err != nil {
			return nil, err
		}
